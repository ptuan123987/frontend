...
const Signup = () => {
  ...
  const handleSignup = (event) => {
    ...
    setForm({ ...form, errors });
    console.log("type of display_name:", typeof form.display_name);

    if (Object.keys(errors).length > 0) {
      return;
    }
    AuthService.register(form.display_name, form.email, form.password).then(
      (response) => {
        navigate("/login", {
      ...
      <section className="my-10 max-w-[21rem] mx-auto">
        ...
        <div className="flex flex-col gap-2">
          <form className="flex flex-col gap-2" onSubmit={handleSignup}>
            ...
            <Button3 className="mt-2" type="submit" onClick={handleSignup}>
              Sign up
            </Button3>
          </form>
          <p className="text-xs">
