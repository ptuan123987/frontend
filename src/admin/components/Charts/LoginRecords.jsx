...
const LoginRecords = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (selectedType === 'daily') {
                    url = `${API_URL}api/analytics/daily-login?days=${selectedPeriod}`;
                } else if (selectedType === 'monthly') {
                    url = `${API_URL}api/analytics/monthly-login?months=${selectedPeriod}`;
                } else if (selectedType === 'yearly') {
                    url = `${API_URL}api/analytics/yearly-login?years=${selectedPeriod}`;
                }
                ...
                const logins_records = response.data.logins_records;
                console.log(logins_records);
                setChartData({
                    labels: Object.keys(logins_records),
                    datasets: [
                        {
                            label: 'Logins Records',
                            data: Object.values(logins_records),
                            fill: false,
    ...
    return (
        <div className="relative ">
            <h2 className="text-2xl font-bold mb-4">Login Analytics</h2>
